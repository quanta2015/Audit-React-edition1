  ...
  doUpload= async (type,e) => {
    ...
    const columns = [{
        ...
        render: (text, record, index) => (
          < >
            {(record.status != 0) && (record.status != 2) && <Button type="default" icon="file" onClick={this.doEdit.bind(this,record)}>详情</Button> }
            {(record.status == 0) && <Button type="primary" icon="edit" onClick={this.doEdit.bind(this,record)}>开始编辑</Button> }
            {(record.status == 2) && <Button className="c-orange" type="primary" icon="edit" onClick={this.doEdit.bind(this,record)}>开始编辑</Button> }
          </>
        ),
      },
    ];
